...
# Define arrays
y_values      = [ [] for m in range(len(cases_dat)) ]
u_mean_values = [ [] for m in range(len(cases_dat)) ]
v_mean_values = [ [] for m in range(len(cases_dat)) ]
w_mean_values = [ [] for m in range(len(cases_dat)) ]

for i in range(len(cases_dat)):
    
    # x planes
    case_i_x02p5 = cases_dat[i]+'probes_turb_gas_plane_x02p5/'
    case_i_x05   = cases_dat[i]+'probes_turb_gas_plane_x05/'
    case_i_x10   = cases_dat[i]+'probes_turb_gas_plane_x10/'
    case_i_x15   = cases_dat[i]+'probes_turb_gas_plane_x15/'
...
# SPS
i = 0; plt.plot(y_values[i][j][k],u_to_plot[i][j][k],formats_dat[i], label=labels_cases_dat[i])
...
# filter shit
y_to_filter = y_values[i][j][k]
u_to_filter = u_to_plot[i][j][k]
...
plt.plot(y_values[i][j][k],u_filtered,formats_dat[i], label=labels_cases_dat[i])
...
plt.legend(loc='best')
plt.tight_layout()
plt.savefig(folder_manuscript+'custom_line_x05_z01p6_ux_mean_along_y.pdf')
plt.show()
plt.close()
...
# SPS
i = 0; 
ax1.plot(y_values[i][j][k],u_to_plot[i][j][k],formats_dat[i], label=labels_cases_dat[i])
...
# Second, apply the filter
u_filtered = signal.filtfilt(B,A, u_to_filter)
ax1.plot(y_values[i][j][k],u_filtered,formats_dat[i], label=labels_cases_dat[i])
...
k = -2 # z = 5 mm
# SPS
i = 0; ax2.plot(y_values[i][j][k],u_to_plot[i][j][k],formats_dat[i], label=labels_cases_dat[i])
#custom 
i = 1; ax2.plot(y_values[i][j][k],u_filtered,formats_dat[i], label=labels_cases_dat[i])
...
#plt.legend(loc='best')
plt.tight_layout()
plt.savefig(folder_manuscript+'custom_line_x10_z01p6_ux_mean_along_y.pdf')
plt.show()
plt.close()
