...
#%% CASE TO CHECK LGS_sprays_beginning_CERFACS                 
...
dirs       = ["dx10_x05mm_wRMS" , 
              "dx10_x05mm_wRMS_FDC_times_01p5"]
...
for i in range(len(sprays_list)):
    SMD_n = sprays_list[i][0].SMD
    print(f'  SMD for case {labels_[n]}: {SMD_n}')
    n += 1
print('\n')

dirs_grid = [ [d] for d in dirs]
grids_list_cases01 = get_discrete_spray(True, sprays_list, [8]*2, 
                                None, params_simulation ,
                                DIR = dirs_grid)


for grid in grids_list_cases01:
    grids_list.append(grid)
    

#%% CASE TO CHECK LGS_sprays_custom_inlet_v02                 
...
dirs_grid = [ [d] for d in dirs]
grids_list_cases02 = get_discrete_spray(True, sprays_list, [8]*2, 
                                None, params_simulation ,
                                DIR = dirs_grid)


for grid in grids_list_cases02:
    grids_list.append(grid)
